...
class vlad:
  ...
  def find_cluster(self, dataset, no_vocab):
    dataset_descriptors = []
  ...
  def save_dataset_vlad(self, dataset, out_path):
    vlads = np.zeros([len(dataset), self.vocabs.shape[0],self.vocabs.shape[1]])
    for i, image in enumerate(dataset):
      vlads[i] = self._calculate_VLAD(image)
  ...
  def _calculate_VLAD(self, image) -> np.ndarray:
    if self.kmeans == None:
